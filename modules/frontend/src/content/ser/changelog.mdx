...
### Commented bare yield* statements are lowered correctly

A bare `ts yield*` statement with a comment directly above it lost part of its lowering: the comment rode into the generated effect argument and kept the original `ts yield*` with it, so the component failed at runtime with `yield* (intermediate value) is not iterable` instead of running the effect. The comment is now excluded from the lowered expression, and such statements execute like any other bare `ts yield*`.
...
### Remote failure reports read the request event lazily

The reporting context for unencodable remote failures is now resolved only when a report is actually rendered. Every remote call used to read `ts request`, `ts route`, and `ts url` from the request event merely to describe the few calls that fail, and a request event that restricts those properties could disturb a handler that had already succeeded. A restricted event now neither slows nor breaks successful calls, and a failure report still includes whatever context the event allows.
